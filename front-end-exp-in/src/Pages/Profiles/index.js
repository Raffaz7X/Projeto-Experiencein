import React, { useEffect, useState } from "react";
import { api } from "../../Services/api";
import "./Profiles.css"

export default function Profiles() {
  const [ profiles, setProfiles ] = useState();
  const [currentProfile, setCurrentProfile] = useState();
  const [message, setMessage] = useState();
  const [currentInvitedProfile, seCurrentInvitedProfile] = useState();
  const [invites, setInvites] = useState();
  useEffect(() => {
    api.get("/perfis/").then((resp) => setCurrentProfile(resp.data))
    .catch((error) => console.error(error));
    api
      .get("/perfis/")
      .then((resp) => setProfiles(resp.data))
      .catch((error) => console.error(error));

    api
    .get("/convites/")
    .then(resp => setInvites(resp.data))
    .catch((error) => console.error(error));
  }, []);

  function invite(id) {
    api.post(`/convites/convidar/${id}`)
    .then((resp) => setMessage(resp.data.message))
    .catch(error => console.error(error));

    seCurrentInvitedProfile(id);

  }

  return (
    <div className="profiles">
      <div className="invite">
        { profiles?.map((profile) => 
          profile.id === currentProfile?.id ? null :( 
            <div key={profile.id}>
              <div className="card">
                  <h3>{ profile.nome }</h3>
                  <span>{ profile.email }</span>
                { profile.pode_convidar ? ( 
                  <button className="icon" title="Convidar" 
                    onClick={() => invite(profile.id)}
                    data-profile-id={ profile.id }
                  ></button>
                ) : null }
              </div>
              {profile.id === currentInvitedProfile ? (
                <span>{message} </span>
              ) : null }
            </div>
          ),
        )}
      </div>
      <div className="invitations">
        <h2>Convites</h2>
        { 
          invites?.map((invite) => ( 
            <div className="card">
              <h3>{invite.nome}</h3>
              <button>Aceitar</button>

            </div>
          ))
        }

      </div>
     
    </div>
    );
}
