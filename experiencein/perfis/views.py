...
def convidar(request, *args, **kwargs):
	try:
		...
		raise exceptions.NotFound('NAO FOI ENCONTRADO')
	
	perfil_logado = get_perfil_logado(request)
	if perfil_a_convidar != perfil_logado:
			perfil_logado.convidar(perfil_a_convidar)
			return response.Response({'mensagem': f'convite enviado com sucesso {perfil_a_convidar.email}.'}, status=status.HTTP_201_CREATED)
	raise exceptions.ParseError('Eita voce Nao pode convida o perfil')
...
def aceitar(request, *args, **kwargs):
	...
	try:
		...
		raise exceptions.NotFound('Nao funciono ')
	convite.aceitar()
